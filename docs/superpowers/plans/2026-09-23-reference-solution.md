...
```bash
python -m venv /tmp/dbt_verify_venv
/tmp/dbt_verify_venv/Scripts/python -m pip install --quiet dbt-core dbt-postgres
/tmp/dbt_verify_venv/Scripts/dbt --version
```
...
cp reference-solution/profiles.yml.example /tmp/dbt_profiles/profiles.yml
# Edit /tmp/dbt_profiles/profiles.yml: host=localhost, port=55432, user=training, password=training, dbname=training
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt debug
```
...
```bash
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt show --inline "select * from {{ source('raw', 'customers') }} limit 5" --profiles-dir /tmp/dbt_profiles
```
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt run --select staging --profiles-dir /tmp/dbt_profiles
```
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt test --select staging --profiles-dir /tmp/dbt_profiles
```
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt run --select curated_customer_orders --profiles-dir /tmp/dbt_profiles
```
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt run --select stg_products stg_order_items --profiles-dir /tmp/dbt_profiles
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt run --select curated_product_sales --profiles-dir /tmp/dbt_profiles
...
**Interfaces:**
- Consumes: `curated_customer_orders` (Task 9), `curated_product_sales` (Task 11).
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt build --profiles-dir /tmp/dbt_profiles
```
...
```bash
cd reference-solution
DBT_PROFILES_DIR=/tmp/dbt_profiles /tmp/dbt_verify_venv/Scripts/dbt docs generate --profiles-dir /tmp/dbt_profiles
```

Expected: `Completed successfully`, `target/catalog.json` and `target/manifest.json` are created (don't commit these — they're covered by the existing `target/` gitignore rule).

```bash
ls target/catalog.json target/manifest.json
```
...
rm -rf /tmp/dbt_verify_venv /tmp/dbt_profiles
```
